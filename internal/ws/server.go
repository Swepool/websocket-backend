...
func (s *Server) Run() {
	...
	mux.HandleFunc("/slow-clients", s.recoverMiddleware(s.handleSlowClientStats))
	mux.HandleFunc("/graphql-stats", s.recoverMiddleware(s.handleGraphQLStats))
	...
	// Start slow client metrics logger
	s.wg.Add(1)
	go func() {
		defer s.wg.Done()
		s.logger.Info("Starting slow client metrics logger")
		s.logSlowClientMetrics()
	}()
...
func (s *Server) addClient(client *Client) {
	...
	shardIndex := int(hash(remoteAddr.String()) % uint32(len(s.clientShards)))
	
	// Validate shard index
	if shardIndex >= len(s.clientShards) {
	...
	// Store shard index in client for optimized removal
	client.SetShardIndex(shardIndex)
...
// removeClient removes a client from the server with shard-aware optimization
func (s *Server) removeClient(client *Client) {
	...
	// Get the stored shard index for optimized removal
	shardIndex := client.GetShardIndex()
	
	// Validate shard index
	if shardIndex < 0 || shardIndex >= len(s.clientShards) {
		s.logger.Warn("Invalid shard index, falling back to full scan", map[string]interface{}{
			"shardIndex": shardIndex,
			"maxShards": len(s.clientShards),
			"clientAddr": clientAddr,
		})
		// Fallback to scanning all shards if index is invalid
		s.removeClientFallback(client, clientAddr)
		return
	}
	
	// Direct removal using stored shard index (O(1) optimization)
	shard := s.clientShards[shardIndex]
	if shard == nil {
		s.logger.Error("Shard is nil", map[string]interface{}{
			"shardIndex": shardIndex,
	...
	if found {
		...
		s.logger.Info("Client removed (optimized)", map[string]interface{}{
			"clientAddr": clientAddr,
			"shardIndex": shardIndex,
			"totalClients": newCount,
		})
	} else {
		s.logger.Warn("Client not found in expected shard, trying fallback", map[string]interface{}{
			"shardIndex": shardIndex,
			"clientAddr": clientAddr,
		})
		// Fallback to full scan if not found in expected shard
		s.removeClientFallback(client, clientAddr)
	}
}

// removeClientFallback performs a full scan removal (fallback for edge cases)
func (s *Server) removeClientFallback(client *Client, clientAddr string) {
	var found bool
	...
	for i, shard := range s.clientShards {
		...
		if shard.clients != nil {
			if _, exists := shard.clients[client]; exists {
				delete(shard.clients, client)
				found = true
				s.logger.Debug("Client removed from shard (fallback)", map[string]interface{}{
					"shardIndex": i,
					"clientAddr": clientAddr,
				})
			}
	...
	if found {
		...
		s.logger.Info("Client removed (fallback)", map[string]interface{}{
			"clientAddr": clientAddr,
			"totalClients": newCount,
		})
	} else {
...
func (s *Server) Shutdown() {
	...
	// Close GraphQL client connections
	if s.graphql != nil {
		s.graphql.Close()
		fmt.Printf("[SERVER] GraphQL client connections closed\n")
	}
}

// getSlowClientMetrics returns metrics about slow clients across all shards
func (s *Server) getSlowClientMetrics() map[string]interface{} {
	totalClients := int64(0)
	slowClients := int64(0)
	totalBufferFullEvents := int64(0)
	highQueueUtilization := int64(0) // Clients with >80% queue utilization
	...
	for i, shard := range s.clientShards {
		...
		func() {
			defer func() {
				if err := recover(); err != nil {
					s.logger.Error("Error collecting slow client metrics from shard", map[string]interface{}{
						"shardIndex": i,
						"error":      err,
					})
				}
			...
			for client := range shard.clients {
				...
				stats := client.GetSlowReadStats()
				totalBufferFullEvents += stats.BufferFullCount
				
				if client.GetQueueUtilization() > 0.8 {
					highQueueUtilization++
				}
			}
		}()
	}
	...
	metrics := map[string]interface{}{
		...
		"slowClientPercentage":    float64(slowClients) / float64(totalClients) * 100,
		"totalBufferFullEvents":   totalBufferFullEvents,
		"highQueueUtilization":    highQueueUtilization,
		"slowClientThreshold":     slowClientThreshold,
		"slowClientTimeWindow":    slowClientTimeWindow,
		"autoDisconnectEnabled":   slowClientAutoDisconnect,
	}
	
	return metrics
}

// getDetailedSlowClientInfo returns detailed information about slow clients
func (s *Server) getDetailedSlowClientInfo() []map[string]interface{} {
	var slowClientDetails []map[string]interface{}
	
	// Collect detailed info from all shards
	for i, shard := range s.clientShards {
		if shard == nil {
			continue
		}
		...
		func() {
			defer func() {
				if err := recover(); err != nil {
					s.logger.Error("Error collecting detailed slow client info from shard", map[string]interface{}{
						"shardIndex": i,
						"error":      err,
					})
				}
			...
			for client := range shard.clients {
				...
				// Only include slow clients or those with high queue utilization
				if client.IsSlowClient() || client.GetQueueUtilization() > 0.8 {
					metrics := client.GetClientMetrics()
					slowClientDetails = append(slowClientDetails, metrics)
				}
			}
		}()
	}
	
	return slowClientDetails
}

// logSlowClientMetrics periodically logs slow client metrics
func (s *Server) logSlowClientMetrics() {
	...
	for {
		...
		case <-ticker.C:
			func() {
				defer func() {
					if err := recover(); err != nil {
						s.logger.Error("Error logging slow client metrics", map[string]interface{}{
							"error": err,
						})
					}
				}()
				...
				// Only log if there are clients or slow clients
				if totalClients, ok := metrics["totalClients"].(int64); ok && totalClients > 0 {
					s.logger.Info("Slow client metrics", metrics)
					
					// Log detailed info if there are slow clients
					if slowClients, ok := metrics["slowClients"].(int64); ok && slowClients > 0 {
						detailedInfo := s.getDetailedSlowClientInfo()
						if len(detailedInfo) > 0 {
							s.logger.Warn("Slow client details", map[string]interface{}{
								"slowClients": detailedInfo,
							})
						}
					}
				}
