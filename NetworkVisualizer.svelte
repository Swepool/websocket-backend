<script lang="ts">
  ...
  const COLORS = {
    ...
    chainFaded: "#71717a", // Faded inactive chains
  ...
  const MIN_FADE_LEVEL = 0.3 // Don't fade completely (30% minimum brightness)
    ...
    for (const [chainId, node] of chainNodes) {
      ...
      if (isSelected) {
        ...
        if (node.fadeLevel < 1) {
          ...
          nodeColor = `#${r.toString(16).padStart(2, '0')}${g.toString(16).padStart(2, '0')}${b.toString(16).padStart(2, '0')}`
        } else {
          nodeColor = baseColor
        }
      }
